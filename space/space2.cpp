...
double D_plus_func(double n, double alpha, double beta, double dt, double dx){
    return ((1.0+beta)/2)*dt*d_plus_func(n*dx,alpha)/pow(dx,alpha);
}
...
double D_minus_func(double n, double alpha, double beta, double dt, double dx){
    return ((1.0-beta)/2)*dt*d_minus_func(n*dx,alpha)/pow(dx,alpha);
}
...
void solve_fractional_diff_eq2(int N, int M, int K,  double dx, double dt, double alpha, double beta, double gamma, double t_max ) {
    ...
    for (int n=0; n<N; n++){
        for(int m=0; m<M; m++){
            if(m==n && m==0){
                ...
                A[n][m]=-1.0*g_func(alpha,n-m+1)*D_plus_func(n,alpha,beta,dt,dx);
            }else if(m == n+1){
                A[n][m]=-1.0*g_func(alpha,0)*D_plus_func(n,alpha,beta,dt,dx)-g_func(alpha,2)*D_minus_func(n,alpha,beta,dt,dx);
            }else if(m==n){
                A[n][m]=1.0-g_func(alpha,1)*D_plus_func(n,alpha,beta,dt,dx)-g_func(alpha,1)*D_minus_func(n,alpha,beta,dt,dx);
            }else if(m==n-1){
                A[n][m]=-1.0*g_func(alpha,2)*D_plus_func(n,alpha,beta,dt,dx)-g_func(alpha,0)*D_minus_func(n,alpha,beta,dt,dx);
            }else if(m>=n+2){
                A[n][m]=-1.0*g_func(alpha,m-n+1)*D_minus_func(n,alpha,beta,dt,dx);
            }
        }
    }
    ...
    for (int n = 0; n < N-1; n++) {
        ...
        vector<double> res = multiplyMatrixVector(inverseMatrix(A),sumVectors(c_n,q_n));
        for (int i=1; i<M-1; i++){
            c[n+1][i]=res[i];
        }
    }
